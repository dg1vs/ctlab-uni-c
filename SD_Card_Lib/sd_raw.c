...
uint8_t sd_raw_send_command(uint8_t command, uint32_t arg)
{
    ...
    if ( (command == CMD_GO_IDLE_STATE) && (MISOdefault != 0xff) )
    {
        // There are some SDs which don't start with the I/O line high. 
        // Here we wait until the I/O has switched to high
        for(uint8_t i = 0; i < 10; ++i)
        {
